export const WHATSAPP_URL = "https://wa.me/5492617048032";

export const navLinks = [
  { label: "Inicio", href: "#inicio" },
  { label: "Nosotros", href: "#nosotros" },
  { label: "Servicios", href: "#servicios" },
  { label: "Cómo trabajamos", href: "#proceso" },
  { label: "Casos de éxito", href: "#testimonios" },
  { label: "Contacto", href: "#contacto" },
];

export const valueCards = [
  {
    title: "Contratá mejor",
    text: "Reducimos errores de selección con procesos claros, perfiles bien definidos y evaluación del ajuste cultural.",
  },
  {
    title: "Ordená tu estructura",
    text: "Diseñamos roles, responsabilidades, procesos y organigramas para que el crecimiento no dependa de la improvisación.",
  },
  {
    title: "Desarrollá tu equipo",
    text: "Impulsamos liderazgo, clima laboral, planes de carrera y evaluación de desempeño.",
  },
];

export const serviceGroups = [
  {
    title: "Selección y Headhunting",
    description:
      "Procesos de reclutamiento estratégicos orientados a resultados de negocio.",
    items: [
      "Selección de personal end to end",
      "Búsqueda de perfiles clave",
      "Headhunting",
      "Selección internacional",
      "Entrevistas por competencias",
    ],
  },
  {
    title: "Desarrollo Organizacional",
    description:
      "Estructuras y procesos de gestión para escalar con orden y claridad.",
    items: [
      "Diseño de estructuras eficientes",
      "Organigramas",
      "Descripción de puestos",
      "Planes de carrera",
      "Detección de High Potentials",
    ],
  },
  {
    title: "Cultura, Clima y Liderazgo",
    description:
      "Fortalecemos vínculos, liderazgo y bienestar para mejorar desempeño sostenible.",
    items: [
      "Diagnóstico de clima laboral",
      "Cultura organizacional",
      "Programas de liderazgo",
      "Capacitación in company",
      "Bienestar organizacional",
    ],
  },
  {
    title: "People Analítica y Compensaciones",
    description:
      "Datos y políticas para decisiones de personas más consistentes y estratégicas.",
    items: [
      "Evaluación de desempeño",
      "Indicadores de RRHH",
      "Benchmark salarial",
      "Política salarial",
      "Marca empleadora",
    ],
  },
];

export const companyPainPoints = [
  "No están claros los roles y responsabilidades.",
  "Hay rotación o dificultad para retener talento.",
  "Cuesta encontrar perfiles confiables.",
  "Los procesos de selección dependen demasiado de la intuición.",
  "El equipo creció, pero los procesos quedaron chicos.",
  "Hay problemas de comunicación interna.",
  "Los líderes necesitan mejores herramientas de gestión.",
  "Se toman decisiones de personal sin indicadores.",
];

export const processSteps = [
  {
    title: "Diagnóstico inicial",
    text: "Relevamos la situación actual, necesidades y objetivos del negocio.",
  },
  {
    title: "Definición de estrategia",
    text: "Diseñamos un plan de acción alineado a la realidad de la empresa.",
  },
  {
    title: "Implementación",
    text: "Ejecutamos procesos de selección, desarrollo, estructura o cultura según el caso.",
  },
  {
    title: "Medición",
    text: "Usamos indicadores para evaluar avances y tomar decisiones.",
  },
  {
    title: "Seguimiento",
    text: "Acompañamos la evolución del equipo y ajustamos la estrategia cuando sea necesario.",
  },
];

export const activeSearches = [
  {
    role: "Analista de Recursos Humanos",
    location: "Argentina | Híbrido",
    workday: "Jornada full time",
  },
  {
    role: "Líder Comercial",
    location: "Latinoamérica | Remoto",
    workday: "Jornada full time",
  },
  {
    role: "Coordinador de Operaciones",
    location: "Argentina | Presencial",
    workday: "Jornada full time",
  },
];

export const impactMetrics = [
  { value: "+15", label: "años de experiencia combinada" },
  { value: "+70", label: "empresas acompañadas" },
  { value: "+10", label: "servicios profesionales de Capital Humano" },
  { value: "LATAM + Europa", label: "presencia global" },
];

export const teamMembers = [
  {
    name: "Valentín Coronado",
    role: "Lic. en RRHH · Business Coach · MBA",
    photo: `${import.meta.env.BASE_URL}team/valentin.webp`,
  },
  {
    name: "Belén Mesa",
    role: "Lic. en Psicología · Especialista en RRHH · MBA · Docente",
    photo: `${import.meta.env.BASE_URL}team/belen.webp`,
  },
];

export const testimonials = [
  {
    company: "Empresa de transporte",
    photo: `${import.meta.env.BASE_URL}testimonios/testimonio1.webp`,
  },
  {
    company: "Empresa de software",
    photo: `${import.meta.env.BASE_URL}testimonios/testimonio2.webp`,
  },
  {
    company: "Empresa mayorista de insumos de ferretería",
    photo: `${import.meta.env.BASE_URL}testimonios/testimonio3.webp`,
  },
];

export const clientLogos = [
  { name: "Zeus Impresiones", logo: `${import.meta.env.BASE_URL}logos/zeus.webp` },
  { name: "El Depósito", logo: `${import.meta.env.BASE_URL}logos/eldeposito.webp` },
  { name: "Katsuda", logo: `${import.meta.env.BASE_URL}logos/katsuda.webp` },
  { name: "Pyramis", logo: `${import.meta.env.BASE_URL}logos/pyramis.webp` },
  { name: "EPRE", logo: `${import.meta.env.BASE_URL}logos/epre.webp` },
  { name: "Griffouliere Transportes", logo: `${import.meta.env.BASE_URL}logos/griffouliere.webp` },
];

export const serviceInterestOptions = [
  "Selección de personal",
  "Headhunting",
  "Desarrollo organizacional",
  "Clima laboral",
  "Evaluación de desempeño",
  "People Analítica",
  "Otro",
];

export const companySizeOptions = [
  "1–10 empleados",
  "11–50 empleados",
  "51–200 empleados",
  "+200 empleados",
];

export const urgencyOptions = [
  "Inmediata (esta semana)",
  "Próximo mes",
  "En los próximos 3 meses",
  "Estoy explorando opciones",
];
